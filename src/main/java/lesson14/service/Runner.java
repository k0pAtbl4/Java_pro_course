...
import static lesson14.Main.printCars;
        ...
        public void run() {
            ...
            for (int i = 0; i < lines.length; i++) {
                ...
                switch (carsInStr[i][0]) {
                    case "FREIGHT" ->
                            cars.push(new FreightCar(Double.parseDouble(carsInStr[i][1]), Double.parseDouble(carsInStr[i][2]), Double.parseDouble(carsInStr[i][3]),
                    ...
                    case "SEDAN" ->
                            cars.push(new SedanCar(Double.parseDouble(carsInStr[i][1]), Double.parseDouble(carsInStr[i][2]), Double.parseDouble(carsInStr[i][3]),
                                    carsInStr[i][4], carsInStr[i][5], Double.parseDouble(carsInStr[i][6]), Boolean.parseBoolean(carsInStr[i][7])));
                    case "SPORT" ->
                            cars.push(new SportCar(Double.parseDouble(carsInStr[i][1]), Double.parseDouble(carsInStr[i][2]), Double.parseDouble(carsInStr[i][3]),
                                    carsInStr[i][4], carsInStr[i][5], Double.parseDouble(carsInStr[i][6]), Boolean.parseBoolean(carsInStr[i][7])));
                }
            }
            ...
            toWrite += Arrays.toString(park.getCars().toArray()) + "\n";
            park.lambdaFuelSort();
            ...
            LinkedList<Car> fitsTheRange = park.fitsRange(minSpeed, maxAllowedSpeed);
